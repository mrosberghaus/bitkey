...
import build.wallet.bitcoin.verificationhash.AddressVerificationHash
import build.wallet.bitcoin.verificationhash.AddressVerificationChallenge
import build.wallet.bitcoin.verificationhash.AddressVerificationHashService
import build.wallet.bitcoin.verificationhash.VerificationMessage
import build.wallet.bitcoin.verificationhash.VerificationMessageError
import build.wallet.bitcoin.verificationhash.CompactEcdsaSignature
import build.wallet.bitcoin.verificationhash.HwVerificationHashSigner
...
import build.wallet.statemachine.verificationhash.AddressVerificationHashUiStateMachineImpl.State.ConfirmOnHardware
import build.wallet.statemachine.verificationhash.AddressVerificationHashUiStateMachineImpl.State.Done
import build.wallet.statemachine.verificationhash.AddressVerificationHashUiStateMachineImpl.State.EnterMessage
import build.wallet.statemachine.verificationhash.AddressVerificationHashUiStateMachineImpl.State.LoadingUsedAddresses
import build.wallet.statemachine.verificationhash.AddressVerificationHashUiStateMachineImpl.State.NoUsedAddresses
import build.wallet.statemachine.verificationhash.AddressVerificationHashUiStateMachineImpl.State.SelectUsedAddress
import build.wallet.statemachine.verificationhash.AddressVerificationHashUiStateMachineImpl.State.ShowingError
...
class AddressVerificationHashUiStateMachineImpl(
  private val bitcoinWalletService: BitcoinWalletService,
  private val addressVerificationHashService: AddressVerificationHashService,
  private val nfcConfirmableSessionUiStateMachine: NfcConfirmableSessionUiStateMachine,
  private val sharingManager: SharingManager,
  private val clipboard: Clipboard,
) : AddressVerificationHashUiStateMachine {
  @Composable
  override fun model(props: AddressVerificationHashUiProps): ScreenModel {
    ...
    return when (val current = state) {
      is LoadingUsedAddresses -> {
        LaunchedEffect("load-used-script-pubkeys") {
          ...
          wallet.listUsedScriptPubKeys()
            .logFailure { "Failed to list used scriptPubKeys for verification hash" }
            .onSuccess { used ->
        ...
        LoadingSuccessBodyModel(
          onBack = props.onExit,
          state = Loading,
          id = VerificationHashEventTrackerScreenId.LOADING_USED_ADDRESSES
        ).asRootScreen()
      }
      ...
      is NoUsedAddresses -> ErrorFormBodyModel(
        ...
        onBack = props.onExit,
        eventTrackerScreenId = VerificationHashEventTrackerScreenId.NO_USED_ADDRESSES,
        errorData = ErrorData(
          segment = SettingsAppSegment.VerificationHash,
          actionDescription = "Listing used addresses for verification hash",
      ...
      is EnterMessage -> {
        ...
        EnterVerificationMessageBodyModel(
          onBack = {
          ...
          onContinue = {
            val message = VerificationMessage.create(current.draft).get()
              ?: return@EnterVerificationMessageBodyModel
      ...
      is Done -> {
        val encodedHex = current.verificationHash.encode().hex()
        VerificationHashDoneBodyModel(
          addressLabel = current.verificationHash.address.address,
          message = current.verificationHash.message.value,
      ...
      is ShowingError -> ErrorFormBodyModel(
        ...
        onBack = props.onExit,
        eventTrackerScreenId = VerificationHashEventTrackerScreenId.VERIFICATION_HASH_ERROR,
        errorData = ErrorData(
          segment = SettingsAppSegment.VerificationHash,
          actionDescription = "Verification hash",
          cause = current.cause
        )
      ).asRootScreen()
    }
      ...
      NfcConfirmableSessionUIStateMachineProps(
        ...
        onSuccess = { signatureBytes: ByteString ->
          ...
          if (hwSignature == null) {
            ...
            val hwSigner = HwVerificationHashSigner { _, _, _, _ -> Ok(hwSignature) }
            addressVerificationHashService
              ...
              .logFailure { "Verification hash failed after HW signature" }
              .onSuccess { verificationHash ->
                setState(Done(verificationHash = verificationHash))
              }
              .onFailure { error ->
        ...
        shouldLock = true,
        segment = SettingsAppSegment.VerificationHash,
        actionDescription = "Signing verification hash on hardware",
        screenPresentationStyle = ScreenPresentationStyle.Root,
        eventTrackerContext = NfcEventTrackerScreenIdContext.ADDRESS_VERIFICATION_HASH,
        confirmationContent = HardwareConfirmationContent.AddressVerificationHash,
  ...
  private fun messageValidationError(draft: String): String? {
    ...
    return when (val error = VerificationMessage.create(draft).getError()) {
      null -> null
      is VerificationMessageError.Blank -> "Message can't be blank"
      is VerificationMessageError.TooLong ->
        "Message must be ${VerificationMessage.MAX_LENGTH} characters or fewer"
    }
  }
  ...
  private sealed interface State {
    ...
    data class Done(val verificationHash: AddressVerificationHash) : State
