...
class AppSpendingDigestSignerImplTests : FunSpec({
  ...
  test("AddressVerificationHashService create with real app signer and fake HW") {
    ...
    val hwSigner = HwVerificationHashSignerFake()
    val service = AddressVerificationHashServiceImpl(signer)
    val verificationHash = service.create(usedSpk, message, hwSigner).shouldBeOk()

    ecdsa.verifyDigest(verificationHash.digest, verificationHash.appSignature, childPublicKey) shouldBe true
    ecdsa.verifyDigest(verificationHash.digest, verificationHash.hwSignature, childPublicKey) shouldBe false
  }
