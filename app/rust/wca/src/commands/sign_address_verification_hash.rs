...
use crate::{
    errors::CommandError,
    fwpb::{SignAddressVerificationHashCmd, Status},
...
#[derive(Debug, Clone)]
pub enum SignAddressVerificationHashResult {
    ConfirmationPending {
...
fn sign_address_verification_hash(
    digest: Vec<u8>,
    change: u32,
    address_index: u32,
    address: String,
    message: String,
) -> Result<SignAddressVerificationHashResult, CommandError> {
    let apdu: apdu::Command = SignAddressVerificationHashCmd {
        digest,
        change,
        address_index,
        address,
    ...
    if Status::try_from(wallet_rsp.status) == Ok(Status::ConfirmationPending) {
        return Ok(SignAddressVerificationHashResult::ConfirmationPending {
...
command!(SignAddressVerificationHash = sign_address_verification_hash -> SignAddressVerificationHashResult,
    digest: Vec<u8>,
    change: u32,
    address_index: u32,
    address: String,
...
mod tests {
    ...
    use super::{SignAddressVerificationHash, SignAddressVerificationHashResult};
    ...
    fn confirmation_pending() -> Result<(), CommandError> {
        let command = SignAddressVerificationHash::new(
            vec![0x11; 32],
            0,
            1,
            "tb1qtest".to_string(),
        ...
        match command.next(response) {
            Ok(State::Result {
                value:
                    SignAddressVerificationHashResult::ConfirmationPending {
                        response_handle: rh,
