...
} bip322_sighash_session_t;

static SHARED_TASK_BSS bip322_sighash_session_t aa_session = {0};
...
void bip322_sighash_handle_init(ipc_ref_t* message) {
  ...
  bip322_sighash_clear_session();

  fwpb_sign_bip322_sighash_cmd* acmd = &cmd->msg.sign_bip322_sighash_cmd;
  ...
  uint8_t token = 1;
  if (confirmation_manager_create(CONFIRMATION_TYPE_SIGN_BIP322_SIGHASH, &token, sizeof(token),
  ...
  display_params.which_action =
    fwpb_display_params_privileged_action_confirm_bip322_sighash_tag;
  strncpy(display_params.action.confirm_bip322_sighash.address, aa_session.address,
          sizeof(display_params.action.confirm_bip322_sighash.address) - 1);
  strncpy(display_params.action.confirm_bip322_sighash.message, aa_session.message,
          sizeof(display_params.action.confirm_bip322_sighash.message) - 1);
...
static bool aa_confirmation_result_handler(ipc_ref_t* message) {
  ...
  if (aa_session.signed_ok) {
    rsp->msg.get_confirmation_result_rsp.which_result =
      fwpb_get_confirmation_result_rsp_sign_bip322_sighash_result_tag;
    memcpy(rsp->msg.get_confirmation_result_rsp.result.sign_bip322_sighash_result.signature
             .bytes,
           aa_session.signature, ECC_SIG_SIZE);
    rsp->msg.get_confirmation_result_rsp.result.sign_bip322_sighash_result.signature.size =
      ECC_SIG_SIZE;
...
void bip322_sighash_register_handlers(void) {
  confirmation_manager_register_result_handler(CONFIRMATION_TYPE_SIGN_BIP322_SIGHASH,
